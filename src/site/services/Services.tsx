...
// ── Service Modal ────────────────────────────────────────────────────────────
function ServiceModal({ svc, onClose }: { svc: DisplayService; onClose: () => void }) {
        ...
        <div className="p-5 md:p-9">
          <div className="mb-6 flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
            ...
            <div className="shrink-0 rounded-2xl bg-[#fdf6f9] px-4 py-3 text-left md:text-right">
              ...
              <p className="m-0 text-3xl font-black leading-none" style={{ color: PINK }}>
                {svc.price}
              </p>
            </div>
          </div>
              ...
              {svc.details.map((d, i) => (
                ...
                >
                  <span
                    ...
                    style={{ background: "linear-gradient(135deg, rgba(233,30,140,0.15), rgba(233,30,140,0.25))" }}
                  >
          ...
          <div
            ...
            style={{ background: "linear-gradient(135deg, #fff0f7 0%, #fce4ec 100%)", borderLeftColor: PINK }}
          >
...
// ── Service Card ─────────────────────────────────────────────────────────────
function ServiceCard({ svc, onOpen }: { svc: DisplayService; onOpen: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "18px 20px 22px" }}>
        ...
        <p style={{ fontSize: 13.5, color: "#888", lineHeight: 1.6, marginBottom: 16 }}>{svc.desc}</p>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <span style={{ fontWeight: 900, fontSize: 17, color: PINK }}>{svc.price}</span>
          </div>
          <motion.button
            onClick={(e) => {
              e.stopPropagation();
...
function PackageCard({ pkg, index }: { pkg: typeof packages[0]; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Photo side */}
      <div
        className="services-package-image"
        style={{
          width: "40%",
          flexShrink: 0,
          position: "relative",
      ...
      >
        ...
        >
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.8)", margin: "0 0 2px", letterSpacing: "0.08em" }}>
            INVESTISSEMENT
          </p>
      ...
      {/* Content side */}
      <div
        className="services-package-content"
        style={{
          flex: 1,
          padding: "40px 44px",
          display: "flex",
...
function TimelineStep({ step, index, total }: { step: typeof timeline[0]; index: number; total: number }) {
  ...
  return (
    <div
      className="services-timeline-step"
      ref={ref}
...
function transformService(apiService: Service): DisplayService {
  // Récupérer l'URL de la première image depuis le tableau images
  const firstImage = apiService.images && apiService.images.length > 0
    ? apiService.images[0].url
    : apiService.image_url || apiService.image_principale || photo8;

  // Construire la galerie à partir des images
  const galleryImages = apiService.images && apiService.images.length > 0
    ? apiService.images.map(img => img.url)
    : [firstImage];

  // Formater le prix
  const formattedPrice = apiService.prix_indicatif
    ? `${apiService.prix_indicatif.toLocaleString()} €`
    : "Sur devis";

  // Déterminer l'icône en fonction du nom ou du slug
  ...
  const iconKey = apiService.slug?.toLowerCase() || apiService.nom?.toLowerCase() || "";
  let icon = "sparkle";
  ...
  // Construire les détails à partir de la description complète
  const details = apiService.description_complete
    ? apiService.description_complete.split("\n").filter(line => line.trim().length > 0)
    : ["Service personnalisé selon vos besoins"];

  return {
    id: apiService.id.toString(),
    title: apiService.nom || "Service",
    desc: apiService.description_courte || apiService.description_complete?.slice(0, 120) || "Service de qualité pour votre mariage.",
    ...
    details,
    testimonial: {
      text: "Un service exceptionnel qui a rendu notre journée inoubliable !",
      author: "Nos clients satisfaits",
    },
    duration: "Sur mesure",
    guests: "Tout format",
    tag: apiService.statut === "ACTIF" ? "Disponible" : "Indisponible",
  };
}

// ── Main export ───────────────────────────────────────────────────────────────
export default function WeddingServices() {
  const [modalSvc, setModalSvc] = useState<DisplayService | null>(null);
  ...
  const {
    ...
    queryKey: ["services"],
    queryFn: () => serviceProvider.findAll(),
  });


  // Transform API data to DisplayService type
  const services: DisplayService[] = apiServices.length > 0
    ? apiServices.map(transformService)
    : defaultServices;
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 15.5, color: "#666", lineHeight: 1.75, maxWidth: 380 }}>
              Découvrez tous les services dont vous avez besoin pour faire de votre mariage un moment
          ...
          >
            ...
            >
              ...
              <div>
                <p style={{ fontSize: 10, color: "rgba(255,255,255,0.8)", margin: "0 0 1px" }}>
                  Satisfaction
                </p>
      ...
      >
        <div
          className="services-section-wrap services-stats-wrap"
          style={{ maxWidth: 1140, margin: "0 auto", padding: "28px 48px" }}
        >
          <motion.div
          ...
          >
            {stats.map((s, i) => (
              ...
              >
                ...
                <p style={{ fontSize: 26, fontWeight: 900, color: PINK, margin: "0 0 2px", lineHeight: 1 }}>
                  {s.num}
                </p>
      ...
      <div className="services-section-wrap" style={{ maxWidth: 1140, margin: "0 auto", padding: "88px 48px 70px" }}>
        ...
        {isLoading ? (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            ...
            />
            <p style={{ color: "#888", fontSize: 16 }}>Chargement des services...</p>
          </div>
        ) : isError ? (
          <div style={{ textAlign: "center", padding: "40px 20px", color: "#e74c3c" }}>
            ...
            <p style={{ fontSize: 13, color: "#999", marginTop: 8 }}>
              Affichage des services par défaut.
            </p>
          ...
          >
            {services.map((svc, i) => (
              <ServiceCard key={svc.id || i} svc={svc} onOpen={() => setModalSvc(svc)} />
            ))}
          </motion.div>
        )}
      </div>
      ...
      {/* ── BOTTOM CTA ───────────────────────────────────────────────────── */}
      <div
        className="services-section-wrap services-bottom-wrap"
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 14.5, color: "#777", lineHeight: 1.7, marginBottom: 24 }}>
              Notre équipe de conseillers dédiés est à votre écoute pour vous accompagner tout au long de
              votre projet.
            </p>
          ...
          >
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                ...
                <div>
                  <h4 style={{ fontSize: 22, fontWeight: 900, color: "#fff", margin: 0, letterSpacing: "-0.02em" }}>
                    Offre spéciale
                  </h4>
              ...
              <p style={{ fontSize: 14.5, color: "rgba(255,255,255,0.88)", lineHeight: 1.65, marginBottom: 22 }}>
                Profitez de <strong>10% de réduction</strong> sur l'ensemble de nos prestations pour toute
                réservation avant le <strong>30 Juin 2025</strong>.
              </p>
              <motion.button

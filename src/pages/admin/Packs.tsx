...
import type { Pack, Service } from "../../lib/admin/api";
...
export default function Packs() {
  ...
  useEffect(() => {
    async function loadPacksAndServices() {
      try {
        ...
        console.error(err);
        setError(err.message || "Impossible de charger les packs et services.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const openEdit = (p: Pack) => {
    ...
    setImagePreview(
      p.image_principale
        ? `http://localhost:8000/storage/${p.image_principale}`
        : null,
    );
    setModal("edit");
  };
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Erreur lors de la sauvegarde du pack.");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Forfaits mariage combinant plusieurs services
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {error ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {packs.length === 0 ? (
            ...
            packs.map((p) => {
              const imageUrl = p.image_principale
                ? `/storage/${p.image_principale}`
                  ...
                  <div>
                    <div className="h-40 overflow-hidden relative">
                      ...
                      <div className="absolute top-3 right-3">
                        <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-1 rounded-full text-[10px] font-semibold">
                          Actif
                        </span>
                      </div>
                    </div>
